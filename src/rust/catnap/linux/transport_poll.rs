...
#[cfg_attr(target_os = "macos", path = "../linux/active_socket.rs")]
mod active_socket;
#[cfg_attr(target_os = "macos", path = "../linux/passive_socket.rs")]
mod passive_socket;
#[cfg_attr(target_os = "macos", path = "../linux/socket.rs")]
mod socket;
...
impl SharedCatnapTransport {
  ...
  pub fn new(config: &Config, runtime: &mut SharedDemiRuntime) -> Result<Self, Fail> {
    ...
    let poller = match Poller::new() {
      Ok(p) => p,
      Err(e) => panic!("could not create epoll socket: {:?}", e),
    };
    if !poller.supports_level() {
      panic!("Platform does not support level poll-mode");
    }
  ...
  /// these per socket.
  fn register_epoll(&mut self, sd: &SockDesc, readable: bool, writable: bool) -> Result<(), Fail> {
    let fd: RawFd = self.raw_fd_from_sd(sd);
    let event = Event::new(*sd, readable, writable);
    unsafe {
      self.poller.add_with_mode(fd, event, PollMode::Level)?;
    }
    Ok(())
  }

  /// THis function removes the handlers for incoming and outgoing I/O on the socket.
  fn unregister_epoll(&mut self, sd: &SockDesc) -> Result<(), Fail> {
    let fd: RawFd = self.raw_fd_from_sd(sd);
  ...
  async fn poll(&mut self) {
    ...
    // println!("#poll {}", std::backtrace::Backtrace::capture());
...
impl NetworkTransport for SharedCatnapTransport {
  ...
  fn socket(&mut self, domain: Domain, typ: Type) -> Result<Self::SocketDescriptor, Fail> {
    ...
    let sd: Self::SocketDescriptor = match typ {
      ...
      Type::DGRAM => {
        ...
        self.register_epoll(&new_sd, true, true)?;
        new_sd
      },
  ...
  fn listen(&mut self, sd: &mut Self::SocketDescriptor, backlog: usize) -> Result<(), Fail> {
    ...
    self.register_epoll(&sd, true, false)?;
  ...
  async fn connect(&mut self, sd: &mut Self::SocketDescriptor, remote: SocketAddr) -> Result<(), Fail> {
    ...
    self.register_epoll(&sd, true, true)?;
  ...
  async fn close(&mut self, sd: &mut Self::SocketDescriptor) -> Result<(), Fail> {
    ...
    match data.deref_mut() {
      SocketData::Active(_) => {
        // self.unregister_epoll(sd, (libc::EPOLLIN | libc::EPOLLOUT) as u32)?
        self.unregister_epoll(sd)?;
      },
      SocketData::Passive(_) => {
        // self.unregister_epoll(sd, libc::EPOLLIN as u32)?
        self.unregister_epoll(sd)?;
      },
      _ => (),
    };
